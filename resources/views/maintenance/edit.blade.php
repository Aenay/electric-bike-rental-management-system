<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Maintenance</h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('maintenance.update', $maintenance) }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="bike_id" class="w-full rounded border-gray-300">
                                @foreach ($bikes as $b)
                                    <option value="{{ $b->id }}" @selected(old('bike_id', $maintenance->bike_id)==$b->id)>#{{ $b->id }} - {{ $b->model }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input name="issue" value="{{ old('issue', $maintenance->issue) }}" class="w-full rounded border-gray-300" />
                            <x-input-error :messages="$errors->get('issue')" class="mt-2" />
                        </div>
                        <div>
                            <label class="block mb-1">Repair Date</label>
                            <input type="date" name="repair_date" value="{{ old('repair_date', optional($maintenance->repair_date)->format('Y-m-d')) }}" class="w-full rounded border-gray-300" />
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full rounded border-gray-300">
                                @foreach (['open','in_progress','done'] as $opt)
                                    <option value="{{ $opt }}" @selected(old('status', $maintenance->status)===$opt)>{{ $opt }}</option>
                                @endforeach
                            </select>
