<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">New Bike</h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('bikes.store') }}" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block mb-1">Model</label>
                            <input name="model" value="{{ old('model') }}" class="w-full rounded border-gray-300" />
                        ...
                        <div>
                            ...
                            <select name="station_id" class="w-full rounded border-gray-300">
                                ...
                                @foreach ($stations as $s)
                                    <option value="{{ $s->id }}" @selected(old('station_id')==$s->id)>{{ $s->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="battery_level" value="{{ old('battery_level', 100) }}" class="w-full rounded border-gray-300" />
